...
typ_name(t::TypeConstructor) = typ_name(t.body)
typ_name(t::DataType) = string(t.name.name)
...
function store_type(sname::Binding, t::TypeConstructor)
    suptype = t.body
    subtypes = store_type(isa(suptype, DataType) ? Binding(suptype.name) : Binding(Main, typ_name(suptype::Union)), suptype)
